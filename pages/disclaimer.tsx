...
const Disclaimer: NextPage<IPage> = (props: any) => {
  const { data, disclaimerData, pageHeading, breadcrumbsData, error, errorMessage } = props;
  const [show, setShow] = useState(false);
  const [open, setOpen] = useState(false);
  const { deviceType } = useDeviceType();
  const [isSelected, setIsSelected] = useState('disclaimer');
  ...
  return (
    <div itemScope itemType="https://schema.org/WebPage">
      <Layout
        seoData={QuickLinks?.fields?.pageSpecificSEOdata[0]}
        footerData={footer}
        headerData={header}
                ...
                {deviceType !== 'desktop' && (
                  <div className={`section-row`}>
                    ...
                    <div className={styles.staticPage}>
                      <PageContent descriptionData={disclaimerData} />
                    </div>
                  </div>
                )}
                {deviceType === 'desktop' ? (
...
export const getServerSideProps: GetServerSideProps = async () => {
  try {
    ...
    const quicklinksData = getQuicklinksData('disclaimer', data);
    const quicklinksBreadcrumbs = getQuicklinksBreadcrumbs('disclaimer', data);

    return {
      props: {
        data,
        disclaimerData: quicklinksData?.quickLinksData,
