...
const PrivacyPolicy: NextPage<IPage> = (props: any) => {
  const { data, privacyData, pageHeading, breadcrumbsData, error, errorMessage } = props;

  const [show, setShow] = useState(false);
  const [open, setOpen] = useState(false);
  const { deviceType } = useDeviceType();
  const [isSelected, setIsSelected] = useState('privacyPolicy');
  ...
  return (
    <div itemScope itemType="https://schema.org/WebPage">
      <Layout
        seoData={QuickLinks?.fields?.pageSpecificSEOdata[1]}
        footerData={footer}
        headerData={header}
                ...
                {deviceType !== 'desktop' && (
                  <div className={`section-row`}>
                    ...
                    <div className={styles.staticPage}>
                      <PageContent descriptionData={privacyData} />
                    </div>
                  </div>
                )}
                {deviceType === 'desktop' ? (
...
export const getServerSideProps: GetServerSideProps = async () => {
  try {
    ...
    const quicklinksData = getQuicklinksData('privacy', data);
    const quicklinksBreadcrumbs = getQuicklinksBreadcrumbs('privacy', data);
    return {
      props: {
        data,
        privacyData: quicklinksData?.quickLinksData,
