...
const CookiePolicy: NextPage<IPage> = (props) => {
  const { data, cookieData, pageHeading, breadcrumbsData, error, errorMessage } = props;

  const [show, setShow] = useState(false);
  const [open, setOpen] = useState(false);
  const { deviceType } = useDeviceType();
  const [isSelected, setIsSelected] = useState('cookiePolicy');
  ...
  return (
    <div itemScope itemType="https://schema.org/WebPage">
      <Layout
        seoData={QuickLinks?.fields?.pageSpecificSEOdata[4]}
        footerData={footer}
        headerData={header}
                ...
                {deviceType !== 'desktop' && (
                  <div className={`section-row`}>
                    ...
                    <div className={styles.staticPage}>
                      <PageContent descriptionData={cookieData} />
                    </div>
                  </div>
                )}
                {deviceType === 'desktop' ? (
...
export const getServerSideProps: GetServerSideProps = async () => {
  try {
    ...
    const quicklinksData = getQuicklinksData('cookie', data);
    const quicklinksBreadcrumbs = getQuicklinksBreadcrumbs('cookie', data);
    return {
      props: {
        data,
        cookieData: quicklinksData?.quickLinksData,
